...
import profile from "../../../assets/unsplash_gQtUcKahZoQ.png";
...
export default function Chatbot() {
  ...
  return (
    <div className="fixed bottom-5 right-5">
      <button
        className="rounded-full p-5 bg-slate-200 dark:bg-slate-900"
        onClick={handleButtonClick}
      >
        <ChatBubbleBottomCenterIcon className="w-5 h-5 text-black dark:text-white" />
      </button>
      {isOpen && <ChatMsgDialog />}
    </div>
  );
}
...
export const ChatMsgDialog = () => {
  return (
    <div
      className="fixed bottom-24 rounded-md drop-shadow-md
                    right-5 w-80 h-3/4 p-6 bg-slate-200 dark:bg-slate-900"
    >
      <div className="relative flex flex-col h-full">
        <section className="flex flex-col justify-between items-center">
          ...
          <h1 className="font-semibold">DITA Chatbot</h1>
...
export const TypeMsg = () => {
  ...
  return (
    <div className="absolute bottom-1 flex flex-row w-full justify-between items-center">
      <input className="rounded-full p-2 bg-white dark:bg-slate-900 w-4/5 h-10" />
